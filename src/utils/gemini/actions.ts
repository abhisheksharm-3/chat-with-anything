    ...
    try {
        ...
        if (!result.success) {
          ...
          await supabase
            ...
            .update({
              ...
              processing_error:
                result.error || "Failed to process YouTube transcript",
            })
            .eq("id", file.id);
      ...
      return `ERROR: ${
        youtubeError instanceof Error
          ...
          : "Failed to process YouTube transcript"
      }`;
    }
  }
  ...
  if (file.type === "pdf") {
    try {
      ...
      return `ERROR: Failed to process the PDF file. ${
        pineconeError instanceof Error ? pineconeError.message : ""
      }`;
    }
  }
 ...
 * It retrieves context from the associated file (using RAG for documents),
 * sends the user message and context to Gemini, and stores both the user's message
 * and the assistant's response in the database.
 * @param {string} chatId - The ID of the current chat session.
 * @param {string} content - The text content of the user's message.
 * @param {string} [userId] - The ID of the user sending the message for authentication.
...
export async function sendMessage(
  ...
  try {
    ...
    if (chat.file_id) {
      ...
      if (
        (chat.type === "video" || chat.files?.type === "youtube") &&
        chat.files?.url &&
