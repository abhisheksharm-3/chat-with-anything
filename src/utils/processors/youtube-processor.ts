...
export async function processYoutubeVideo(
  ...
  try {
    ...
    try {
      ...
      await supabase
        ...
        .update({
          processing_status: "failed",
          processing_error:
            "No transcript available for this YouTube video. The video might not have captions, or they might be disabled.",
        })
        .eq("id", namespace);
    ...
    console.log(
      `Successfully extracted transcript with ${transcriptText.length} characters`,
    );
    ...
    console.log(
      `Splitting transcript into chunks (size: ${CHUNK_SIZE}, overlap: ${CHUNK_OVERLAP})...`,
    );
    ...
    console.log(
      `Storing transcript chunks in Pinecone with namespace: ${namespace}...`,
    );
    ...
    while (retryCount < MAX_RETRIES && !success) {
      try {
        ...
        success = true;
        console.log(
          `Successfully stored ${chunkedDocs.length} transcript chunks in Pinecone`,
        );
      } catch (storeError) {
        retryCount++;
        console.error(
          `Error storing transcript in Pinecone (attempt ${retryCount}/${MAX_RETRIES}):`,
          storeError,
        );
