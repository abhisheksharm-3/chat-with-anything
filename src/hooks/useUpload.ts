...
import { getAllAcceptedFileTypes, getFileTypeConfig } from "@/constants/FileTypes";
import { getErrorMessage, getUrlType, isValidUrl } from "@/utils/upload-utils";
import { TypeUploadError, TypeUploadStatus, TypeUseUploadLogicProps } from "@/types/upload";
...
export const useUploadLogic = ({
   ...
   */
  const getEnhancedErrorMessage = useCallback((err: unknown): TypeUploadError => {
    // Network errors
    ...
    // Validation errors
    if (err instanceof Error && (err.message.includes('size') || err.message.includes('type') || err.message.includes('format'))) {
      return createUploadError(
        'validation',
        err.message,
        err,
   ...
   */
  const setEnhancedError = useCallback((err: unknown) => {
   ...
   */
  const validateFile = useCallback((file: File): TypeUploadError | null => {
   ...
   */
  const validateUrl = useCallback((urlToValidate: string): TypeUploadError | null => {
  ...
  const handleUrlChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (newUrl.trim()) {
        ...
        if (urlError && newUrl.length > 10) { // Only show error for longer URLs
          setError(urlError);
        } else {
          setError(null);
        }
  ...
  const handleUrlUpload = useCallback(async (): Promise<TypeFile | null> => {
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error("Upload timeout - request took too long")), UploadTimeout);
      });
  ...
  }, [url, fileType, uploadFileAsync, updateFileAsync, validateUrl, setEnhancedError]);
      ...
      for (let i = 0; i <= MaxRetries; i++) {
        try {
          ...
          if (i < MaxRetries) {
            ...
            console.log(`Retrying chat creation in ${delay}ms (${i + 1}/${MaxRetries})...`);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        const timeoutPromise = new Promise<never>((_, reject) => {
          setTimeout(() => reject(new Error("File upload timeout")), UploadTimeout);
        });
